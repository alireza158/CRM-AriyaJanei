    ...
    <x-slot name="header">
        <div class="flex gap-5 items-center" dir="rtl">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ویرایش فاکتور #{{ $invoice->id }}{{ \Illuminate\Support\Facades\Auth::user()->name }} / {{ $invoice->customer->name }} برای
            </h2>
                |
            <h3>
{{--                @dd($customer)--}}
    ...
    <div class="py-8" dir="rtl">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
                <form action="{{ route('marketer.invoices.update', [
//                  'marketer' =>\Illuminate\Support\Facades\Auth::user(),
                  ...
                  'invoice' =>$invoice]) }}"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4"
                                  class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror text-right">{{ old('description', $invoice->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-xs mt-1 text-right">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('marketer.invoices.index', $customer) }}"
