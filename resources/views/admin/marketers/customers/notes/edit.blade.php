<x-layouts.app>
    <x-slot name="header">
        <div class="flex gap-4 items-center" dir="rtl">
            ...
            |
            <h3>
                <a href="{{ route('admin.marketers.customers.notes.show', ['marketer' => $marketer->id, 'customer' => $customer->id, 'note' => $note->id]) }}" class="hover:underline">
                    بازگشت به نمایش
                </a>
            </h3>
            |
    ...
    <div class="py-12" dir="rtl">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('admin.marketers.customers.notes.update', ['marketer' => $marketer->id, 'customer' => $customer->id, 'note' => $note->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="content" id="content" rows="5"
                                  class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-300 text-right" dir="rtl">{{ old('content', $note->content) }}</textarea>
                        @error('content')
                        <p class="text-red-600 text-sm mt-1 text-right">{{ $message }}</p>
                        @enderror
                    </div>
