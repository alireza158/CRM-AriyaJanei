        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
                ...
                <form action="{{ route('marketer.invoices.store', $customer) }}" method="POST" class="text-right" enctype="multipart/form-data">
                    ...
                    {{-- فایل پیوست --}}
                    <div class="mb-6">
                        <label for="attachment" class="block text-gray-700 text-sm font-medium mb-2 text-right">آپلود فایل (اختیاری)</label>
                        <input type="file" name="attachment" id="attachment"
                               class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-right">
                        @error('attachment')
                        <p class="text-red-500 text-xs mt-1 text-right">{{ $message }}</p>
                        @enderror
                        ...
                        @if(isset($invoice) && $invoice->attachment_path)
                            <p class="mt-2">
                                فایل فعلی: 
                                <a href="{{ asset('storage/'.$invoice->attachment_path) }}" target="_blank" class="text-blue-600 hover:underline">
                                    مشاهده فایل
                                </a>
                            </p>
                        @endif
