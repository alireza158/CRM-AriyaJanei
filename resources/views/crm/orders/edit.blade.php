{{-- resources/views/crm/orders/edit.blade.php --}}
<!doctype html>
<html lang="fa" dir="rtl">
@php
  logger()->info('CRM DRAFT EDIT LOADED', [
  ...
  <div class="container page-shell">
    ...
    <div class="topbar mb-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          ...
          <div class="hint">کد پیش‌نویس: <span class="fw-semibold">{{ $order->uuid }}</span></div>
        </div>
      </div>

      <span class="badge badge-soft px-3 py-2">
        وضعیت: {{ $order->status === 'draft' ? 'پیش‌نویس' : $order->status }}
      </span>
    </div>
    ...
    <form action="{{ route('crm.orders.draft.update', $order->uuid) }}{{ request('token') ? ('?token='.request('token')) : '' }}"
      ...
      <div class="card-soft p-3 p-md-4 mb-4">
        ...
        <div class="row g-3 align-items-end">
          <div class="col-lg-6">
            ...
            <select id="shipping_id" name="shipping_id" class="form-select" required>
              ...
              @foreach($shippings as $ship)
                ...
                @elseif($ship['name'] === 'ارسال فوری')
                  <option value="{{ $ship['id'] }}" {{ (int)old('shipping_id', $order->shipping_id) === (int)$ship['id'] ? 'selected' : '' }}>
                    ارسال فوری (فقط داخل استان گلستان)
                  </option>
            ...
            <input type="hidden" id="shipping_price" name="shipping_price" value="{{ (int)old('shipping_price', $order->shipping_price) }}">
          </div>
        ...
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <input type="number"
                   ...
                   id="discount"
                   class="form-control"
                   value="{{ (int)old('discount_amount', $order->discount_amount) }}"
                   readonly
          ...
          <div class="col-md-4">
            ...
            <input type="text"
                   class="form-control"
                   readonly
                   value="با انتخاب شهر محاسبه می‌شود"
                   style="background-color: var(--bs-secondary-bg);">
          </div>
      ...
      <div class="sticky-submit">
        {{-- ✅ ذخیره پیش‌نویس --}}
        <button type="submit" class="btn btn-warning w-100 fs-5 py-3 shadow-sm mb-2">
          💾 ذخیره تغییرات پیش‌نویس
        </button>

        {{-- ✅ ثبت نهایی از روی همین فرم (formaction) --}}
        <button type="submit"
                formaction="{{ route('crm.orders.draft.submit', $order->uuid) }}{{ request('token') ? ('?token='.request('token')) : '' }}"
                formmethod="POST"
                class="btn btn-success w-100 fs-5 py-3 shadow-sm">
          ✅ ثبت نهایی در آریا
        </button>

        <div class="hint text-center mt-2">
          با ثبت نهایی، موجودی آیتم‌ها دوباره بررسی می‌شود.
        </div>
      </div>
...
<script>
  ...
  const initialShippingId  = Number({{ (int)old('shipping_id', $order->shipping_id) }});
  const initialProvinceId  = Number({{ (int)old('province_id', $order->province_id) }});
  const initialCityId      = Number({{ (int)old('city_id', $order->city_id ?? 0) }});
  const initialShipPrice   = Number({{ (int)old('shipping_price', $order->shipping_price) }});
</script>
...
function setShopDeliveryMode(enable) {
  ...
  if (enable) {
    ...
    if (addressEl && !addressEl.value) addressEl.value = 'مراجعه حضوری برای دریافت';
  } else {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ✅ Prefill shipping
  if (initialShippingId) {
    shippingSelect.value = String(initialShippingId);
    selectedShipping = shippings.find(s => Number(s.id) === Number(initialShippingId)) ?? null;
  }

  // ✅ Prefill province/city (ولی چون منطق شما بعد از انتخاب ارسال محدود می‌کند، اینجا فقط قیمت را ست می‌کنیم
  // و سپس با dispatch change منطق خودش تنظیم می‌کند)
  document.getElementById('shipping_price').value = String(initialShipPrice || 0);
  if (initialShipPrice) shippingLabel.textContent = `هزینه ارسال: ${initialShipPrice.toLocaleString()} تومان`;

  // ✅ فعال کردن منطق shipping
  shippingSelect.dispatchEvent(new Event('change', { bubbles: true }));

  // ✅ بعد از اینکه shipping قوانین را اعمال کرد، استان/شهر را ست کنیم
  setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function lockName(lock) {
    ...
    nameInput.classList.toggle('bg-light', lock);
  }
  ...
  // ✅ در صفحه ادیت: اگر موبایل پر است، یک بار چک کن (بدون تغییر نام اگر قبلاً وارد شده)
  const initialMobile = (mobileInput.value || '').trim();
  if (initialMobile.length >= 10) {
    // فقط status را نشان بده
    findCustomer(initialMobile);
  }
});
</script>

<script>
/* -------------------- Products (همان کد خودت + Prefill Draft Items) -------------------- */
let allProducts = [];
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // ✅ اینجا: اگر draft آیتم داشت، همون‌ها را بساز
    if (Array.isArray(draftItems) && draftItems.length) {
      for (const it of draftItems) {
        addProductRow({
          product_id: Number(it.product_id),
          variety_id: Number(it.variety_id),
...
function refreshVarietyOptionLocks() {
  document.querySelectorAll('.product-row').forEach(row => {
    const pId = parseInt(row.querySelector('.product-select')?.value || 0);
    const vSel = row.querySelector('.variety-select');
    if (!pId || !vSel || vSel.disabled) return;
...
function addProductRow(prefill = null) {
  ...
  if (prefill && prefill.product_id) {
    ...
    qtyInput.value = String(prefill.quantity || 1);

    // ست کردن محصول (Select2 همزمان)
    productSelect.value = String(prefill.product_id);
    if (window.jQuery) $(productSelect).trigger('change.select2');
    productSelect.dispatchEvent(new Event('change', { bubbles: true }));

    // بعد از اینکه variety لود شد، مدل را ست کن
    setTimeout(() => {
      const vSel = row.querySelector('.variety-select');
      if (vSel && prefill.variety_id) {
        vSel.value = String(prefill.variety_id);
...
<script>
/* -------------------- Convert digits to int before submit -------------------- */
(function () {
...
<script>
  window.__draftLock = {
    uuid: "{{ $order->uuid }}",
    lockUrl: "{{ route('crm.orders.draft.lock', $order->uuid) }}",
    hbUrl: "{{ route('crm.orders.draft.heartbeat', $order->uuid) }}",
    unlockUrl: "{{ route('crm.orders.draft.unlock', $order->uuid) }}",
    backToList: "{{ route('crm.orders.draft.index') }}",
    csrf: "{{ csrf_token() }}",
  };
</script>
<script src="{{ asset('js/draft-lock.js') }}"></script>
